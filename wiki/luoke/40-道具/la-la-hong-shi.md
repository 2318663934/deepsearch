---
title: 辣辣红柿
type: item
slug: la-la-hong-shi
aliases: []
sources:
- raw\luoke\bilibili-wiki\2026-06-10\rocom_E8BEA3E8BEA3E7BAA2E69FBF.wikitext
confidence: 0.95
confidence_reason: 'B 站 wiki 物品信息模板结构化解析 | 硬规则校准: 无调整'
created: '2026-06-10T19:00:01+08:00'
updated: '2026-06-10T19:00:01+08:00'
last_verified: '2026-06-10T19:00:01+08:00'
status: verified
version: 1
---

“确实辣，但它觉得不辣。”

## 事实
### rarity
- **值**: 紫
- **原文依据**: rarity=紫

### main_category
- **值**: 材料
- **原文依据**: main_category=材料

### sub_category
- **值**: 物攻努力经验材料
- **原文依据**: sub_category=物攻努力经验材料

### use
- **值**: 投喂后提升精灵的物攻努力值。
- **原文依据**: use=投喂后提升精灵的物攻努力值。

### description
- **值**: “确实辣，但它觉得不辣。”
- **原文依据**: description=“确实辣，但它觉得不辣。”

### source
- **值**: 精灵首领·恶魔狼掉落
- **原文依据**: source=精灵首领·恶魔狼掉落
